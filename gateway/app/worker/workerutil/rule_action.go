...
func GetShowErrorPageContent(errorCode string, errorMessage string) string {
...
func ActionShowErrorPage(ctx *gin.Context, httpStatus int, errorCode string, errorMessage string) {
	...
	xSpan := cheerapp.SpanBeginBizFunction(ctx.Request.Context(), "workerutil.ActionShowErrorPage")
	defer func() {
		cheerapp.SpanEnd(xSpan)
	}()

	cheerapp.SpanTag(xSpan, "errorCode", errorCode)

	cheerapp.SpanError(xSpan, errorMessage)

	xPageContent := GetShowErrorPageContent(errorCode, errorMessage)
